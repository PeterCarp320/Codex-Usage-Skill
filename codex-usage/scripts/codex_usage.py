...
def local_path_label(path: Path, codex_home: Path) -> str:
    """Return a portable label without exposing an absolute filesystem path."""
    try:
        relative = path.resolve().relative_to(codex_home.resolve())
    except (OSError, ValueError):
        return "[local path]"
    base = codex_home_label(codex_home)
    return base if str(relative) == "." else f"{base}/{relative.as_posix()}"


def codex_home_label(codex_home: Path) -> str:
...
def build_url(path: str) -> str:
    if not path.startswith("/") or "://" in path:
        raise ValueError("Online endpoints must be backend-relative paths.")
...
def sqlite_threads_summary(codex_home: Path, top_n: int) -> dict[str, Any]:
    ...
    for db_path in candidates:
        ...
        try:
            ...
            basic: dict[str, Any] = {
                "database": local_path_label(db_path, codex_home),
                "rows": 0,
                "tokens_used_sum": 0,
                "tokens_used_max": 0,
                "created_at_min": None,
...
def print_local_usage(data: dict[str, Any], top: int, days: int) -> None:
    ...
    explain(
        "Top sessions are the largest local sessions by total token counter. Project names, paths, and session identifiers are intentionally omitted."
    )
    print_counter_table(
        "Top sessions by total tokens",
        ["Date", "Model", "Total", "Output"],
        top_rows,
    )
